...
class Interpreter:
    ...
    def visit_Set(self, node: Set):
        """Mengubah nilai pada objek (attribute atau index)"""
        # node.target adalah Get node
        target = node.target
        value = self.visit(node.value)
    
        # Evaluasi objek dasar (tanpa akses terakhir)
        obj = self.visit(target.obj)
    
        # Attribute assignment
        if target.attr is not None:
            try:
                # Cek apakah objek punya method 'ubah' (untuk class Indonesian Script)
                if hasattr(obj, 'ubah') and callable(obj.ubah):
                    obj.ubah(target.attr, value)
                else:
                    # Default Python attribute assignment
                    setattr(obj, target.attr, value)
            except AttributeError:
                raise AtributGalat(f"Tidak dapat mengubah atribut '{target.attr}' pada objek {type(obj).__name__}")
        ...
        # Index assignment
            ...
            try:
                ...
                if hasattr(obj, 'ubah') and callable(obj.ubah):
                    ...
                    # Default Python index assignment
                    obj[idx] = value
            except (IndexError, KeyError, TypeError) as e:
                if isinstance(obj, (list, tuple)) and isinstance(idx, int):
                    if idx < 0 or idx >= len(obj):
                        raise IndeksGalat(f"Indeks {idx} di luar jangkauan (panjang: {len(obj)})")
                raise IndeksGalat(f"Tidak dapat mengubah indeks {idx} pada objek {type(obj).__name__}: {e}")
    
        # Update objek di scope (jika perlu)
        # Cari nama variabel asli
        var_name = None
        var_info = {}
        
        if isinstance(target.obj, Variable):
    ...
    def visit_ClassBlock(self, node: ClassBlock):
        [self.visit(method) for method in node.methods if method is not None]
        [self.visit(property) for property in node.properties if property is not None]
    ...
    def visit_Literal(self, node: Literal):
        if isinstance(node.value, dict):
            ...
            for k, v in node.value.items():
                obj[k] = self.visit(v)
            return obj
        elif isinstance(node.value, (list, tuple, set)):
            return [self.visit(v) for v in node.value if v is not None]
        return node.value
    ...
    def visit_Get(self, node: Get):
        """Mengakses nilai dari objek (attribute atau index)"""
        obj = self.visit(node.obj)
    
        # Attribute access
        if node.attr is not None:
            try:
                # Cek apakah objek punya method 'ambil' (untuk class Indonesian Script)
                if hasattr(obj, 'ambil') and callable(obj.ambil):
                    return obj.ambil(node.attr)
                else:
                    # Default Python attribute access
                    return getattr(obj, node.attr)
            except AttributeError:
                raise AtributGalat(f"Tidak ada atribut '{node.attr}' pada objek {type(obj).__name__}")
        ...
        # Index access
            ...
            try:
                # Cek apakah objek punya method 'ambil' (untuk class Indonesian Script)
                if hasattr(obj, 'ambil') and callable(obj.ambil):
                    return obj.ambil(idx)
                else:
                    # Default Python index access
                    return obj[idx]
            except (IndexError, KeyError, TypeError) as e:
                if isinstance(obj, (list, tuple)) and isinstance(idx, int):
                    if idx < 0 or idx >= len(obj):
                        raise IndeksGalat(f"Indeks {idx} di luar jangkauan (panjang: {len(obj)})")
                raise IndeksGalat(f"Tidak dapat mengakses indeks {idx} pada objek {type(obj).__name__}: {e}")
    ...
    def _check_type(self, value, type_ann):
        ...
        if isinstance(type_ann, BasicType):
            expected = TYPES.get(type_ann.name, type_ann.name)
            if expected == callable:
                if not callable(value):
                    raise TipeGalat(f"Nilai {value} tidak sesuai tipe {type_ann.name}")
            else:
                if expected == type_ann.name:
                    obj = self.current_scope.get(type_ann.name)
                    if inspect.isfunction(obj):
                        if not bool(obj(value)):
                            return TipeGalat(f"Nilai {value} tidak sesuai tipe {type_ann.name}")
