...
impl Interpreter {
    ...
    pub fn new(filename: Option<&str>, ismodule: bool) -> Self {
        ...
        let current_scope = lib::Scope::new(None);
...
fn main() {
    // Inisialisasi interpreter baru
    let mut interp = Interpreter::new(None, false);
    
    // Test deklarasi langsung ke global scope interpreter
    interp.current_scope.declare(
        "nama",
        lib::Value::Angka(10),
        "int".to_string(),
        None,
        false,
    ).expect("Gagal deklarasi");
    
    let var = interp.current_scope.get("nama", "name");
    println!("Variabel: {:?}", var);
}
