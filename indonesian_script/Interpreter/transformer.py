...
class ASTBuilder(Transformer):
    ...
    def term(self, items):
        ...
        # prefix adalah expression dasar, postfix adalah getattr/getindex/call
        if len(items) == 1:
            return items[0]
        base = items[0]
        for post in items[1:]:
    ...
    def getattr(self, items):
        ...
        return GetAttr(attr=str(items[0]))
    
    def getindex(self, items):
        # items: [expression]
        return GetIndex(index=items[0])
        
    def call_params(self, items):
        # items adalah list of expression
        if items:
            return CallParameter(args=items[0]) #self._del_list(items))
        else:
